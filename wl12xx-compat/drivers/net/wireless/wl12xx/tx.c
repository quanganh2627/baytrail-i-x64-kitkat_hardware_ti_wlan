					      ...
					      struct sk_buff *skb)
{
	 ...
	 * association response. this way it won't get de-authed by FW
				...
				u32 buf_offset, u8 hlid)
{
	...
	if (total_blocks <= wl->tx_blocks_available) {
		...
		if (wl->chip.id == CHIP_ID_1283_PG20) {
			...
			/* in MCP3 we give the number of extra blocks, not bytes */
			desc->wl128x_mem.extra_bytes = TX_HW_BLOCK_SPARE;
		} else {
