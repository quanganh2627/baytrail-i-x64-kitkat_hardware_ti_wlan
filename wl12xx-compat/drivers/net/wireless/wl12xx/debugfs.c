...
static ssize_t p2p_write(struct file *file,
			   const char __user *user_buf,
			   size_t count, loff_t *ppos)
{
	struct wl1271 *wl = file->private_data;
	...
	ret = strict_strtoul(buf, 0, &value);
	if (ret < 0) {
		wl1271_warning("illegal value in p2p");
		goto out;
	}

	if (value & BIT(0)) {
		wl1271_debug(DEBUG_CMD, "enable p2p role");
		wl1271_cmd_p2p_role_enable(wl);
	}
	if (value & BIT(1)) {
		wl1271_debug(DEBUG_CMD, "disable p2p role");
		wl1271_cmd_p2p_role_disable(wl);
	}
	if (value & BIT(2)) {
		wl1271_debug(DEBUG_CMD, "start p2p role");
		wl1271_cmd_role_start_p2p_dev(wl);
	}
	if (value & BIT(3)) {
		wl1271_debug(DEBUG_CMD, "stop p2p role");
		wl1271_cmd_role_stop_p2p_dev(wl);
	}
	if (value & BIT(4)) {
		wl1271_debug(DEBUG_CMD, "p2p start discovery");
		wl1271_cmd_build_probe_req(wl, "BLABLA", 6,
					 "", 0,
					 IEEE80211_BAND_2GHZ);
		wl1271_cmd_p2p_start_discovery(wl);
	}
...
static const struct file_operations p2p_ops = {
	.write = p2p_write,
	.open = wl1271_open_file_generic,
	.llseek = default_llseek,
};
