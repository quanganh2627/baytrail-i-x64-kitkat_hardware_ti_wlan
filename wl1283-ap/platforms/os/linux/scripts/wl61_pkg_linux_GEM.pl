...
@lin_dirs_open_source = (
             'CUDK/wpa_supplicant',
             'CUDK/wpa_supplicant/openssl-0.9.8e',
             );
...
@lin_files_open_source = (
              'CUDK/wpa_supplicant/.config',
              'CUDK/wpa_supplicant/bufferObj.c',
              'CUDK/wpa_supplicant/bufferObj.h',
              'CUDK/wpa_supplicant/wsc_supplicant.c',
              'CUDK/wpa_supplicant/wsc_supplicant.h',
              'CUDK/wpa_supplicant/CipherWrapper.c',
              'CUDK/wpa_supplicant/CipherWrapper.h',
              'CUDK/wpa_supplicant/eap_wsc.c',
              'CUDK/wpa_supplicant/WscTypes.h',
              'CUDK/wpa_supplicant/eap_wsc.c',
              'CUDK/wpa_supplicant/ec.c',
              'CUDK/wpa_supplicant/ec.h',
              'CUDK/wpa_supplicant/gem.c',
              'CUDK/wpa_supplicant/gem.h',
              'CUDK/wpa_supplicant/wapi.c',
              'CUDK/wpa_supplicant/wapi.h',
              'CUDK/wpa_supplicant/wpa_suppl.diff',
              'CUDK/wpa_supplicant/openssl-0.9.8e/openssl-gcc42.diff',
              'CUDK/wpa_supplicant/openssl-0.9.8e/Makefile',
...
@full_pkg_files_ccx_linux = (@full_pkg_files_noccx_linux, @ccx_files);
...
if($win == 0){
     if($ccx == 0){
        if($gpl == 0){
           ...
           $out_file = 'WiLink61_Linux_noCCX.tar.bz2';
        }
        else {
        ...
        {
           @pkg_dirs = (@full_pkg_dirs_ccx_linux, @lin_dirs, @images_dirs, @lin_dirs_open_source);
           @pkg_files = (@full_pkg_files_ccx_linux, @lin_files, @images_files, @lin_files_open_source);
           $out_file = 'WiLink61_Linux_CCX.tar.bz2';
        }
 }
...
##############################################################################
print "Creating directry structure...\n";

for($i=0; $i<@pkg_dirs; $i+=1)
{
   print " making $pkg_temp_dir/$pkg_dirs[$i]\n";
   system("mkdir $pkg_temp_dir/$pkg_dirs[$i]");
}
